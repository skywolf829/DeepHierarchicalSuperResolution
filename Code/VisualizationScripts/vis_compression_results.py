...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_folder',default="Mixing2D_compression_test",
    ...
    #for metric in results["SZ"].keys():
        #results["SZ"][metric] = np.delete(results["SZ"][metric], np.arange(0, 66))
    ...
    if(not make_all):
        ...
        #Vorts PNSR/SSIM
        #y = np.array([1.0, 31.3, 218.2])
        #x = np.array([58, 39.7, 34])
        #x = np.array([1.0, 0.9594, 0.9026])

        #Plume PNSR/SSIM
        #y = np.array([1.0, 59, 377, 2006])
        #x = np.array([64, 50, 42.88, 37.57])
        #x = np.array([1.0, 0.9970, 0.9831, 0.9476])
