...
import threading
import copy
from utility_functions import save_obj, load_obj, ssim, ssim3D, toImg
        ...
        with torch.no_grad():
            ...
            for i in range(len(torch_models)):
                ...
                self.models.append(torch.jit.trace(torch_models[i], 
                    torch.zeros(torch_models[0].resolution).unsqueeze(0).unsqueeze(0).to(device)))
    ...
    while(m < args['end_metric']):
        ...
        if(args['debug']):           
            ...
            imageio.imwrite(os.path.join(save_folder, save_name+"_point.png"), 
                    toImg(img_upscaled_point[0].cpu().numpy(), mode))
            del img_upscaled_point
        for i in range(len(nodes)):
            del nodes[i].data
        del nodes
